...
impl Action {
    pub async fn execute(&self, event: &Event) -> Result<(), Error> {
        ...
        if self.triggers.iter().any(|ek| eq_triggers(ek, &event.kind)) {
...
// eq_triggers сравнивает тригер взятый в конфиге с полученным
fn eq_triggers(conf_ek: &EventKind, gived_ek: &EventKind) -> bool {
    trace!("conf_ek: {:?}, gived_ek: {:?}", conf_ek, gived_ek);
    match conf_ek {
        EventKind::Any => true,
        EventKind::Access(a) => {
            if a == &AccessKind::Any {
                matches!(gived_ek, EventKind::Access(_))
            } else {
                conf_ek == gived_ek
            }
        }
        EventKind::Create(c) => {
            if c == &CreateKind::Any {
                matches!(gived_ek, EventKind::Create(_))
            } else {
                conf_ek == gived_ek
            }
        }
        EventKind::Modify(m) => {
            if m == &ModifyKind::Any {
                matches!(gived_ek, EventKind::Modify(_))
            } else {
                conf_ek == gived_ek
            }
        }
        EventKind::Remove(r) => {
            if r == &RemoveKind::Any {
                matches!(gived_ek, EventKind::Remove(_))
            } else {
                conf_ek == gived_ek
            }
        }
        EventKind::Other => conf_ek == gived_ek,
    }
}
